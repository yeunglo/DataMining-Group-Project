...
with open(path_full) as Train:
    ...
    for line in reader:
        ...
        tag_str.append((line['toxic']))
        # if line['toxic']=='1' and line['severe_toxic']=='0':
        #     tag_super.append(1)
        # elif line['toxic']=='1' and line['severe_toxic']=='1':
        #     tag_super.append(2)
        # else:
        #     tag_super.append(0)
        tag_multi1=str(line['obscene'])
        tag_multi1+=str(line['threat'])
        tag_multi1+=str(line['insult'])
        tag_multi1+=str(line['identity_hate'])
        tag_multi1=int(tag_multi1,2)
        tag_multi.append(tag_multi1)
#print tag_multi
#print tag_super
# for item in words:
...
def fitandpredicted(x_train, x_test, tag_train, tag_test, penalty='l2',C=1,solver='lbfgs'):
    ...
    #class_weight={ 0:0.1,1:0.9},penalty='l1', solver='lbfgs'
    clf = linear_model.LogisticRegressionCV(multi_class='ovr').fit(x_train, tag_train)
    #clf = LogisticRegression(penalty=penalty,C=C, solver=solver, n_jobs=-1).fit(x_train, tag_train)
    predicted = clf.predict(x_test)
    #pre=clf.predict_proba(x_test)
    #target_names = ['not_toxic', 'toxic', 'serve toxic']
   # print target_names
    #print predicted
...
def split_data(input, random_state=15, shuffle=True):
    input_x, y = input
    #print input_x
    x_train, x_test, tag_train, tag_test = train_test_split(input_x, y, test_size=0.1, random_state=random_state,shuffle=True)
...
def balance_rus(input):
    input_x, input_y = input
    rus = RandomUnderSampler(random_state=42)
    X_res, y_res = rus.fit_sample(input_x, input_y)
    return X_res, y_res
def balance_smote(input):
    input_x, input_y = input
    sm = SMOTE(random_state=42)
    X_res, y_res = sm.fit_sample(input_x, input_y)
    return X_res, y_res
...
input1=[tfidf_matrix,tag_multi]
X_res, y_res=balance_rus(input1)
# n_comp = 100
# svd = decomposition.TruncatedSVD(n_components='n_comp')
X_res=tfidf_matrix.toarray()
# # x=svd.fit(tf)
